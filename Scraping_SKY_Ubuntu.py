...
while True:
	if (tiempo.tm_hour<11 and tiempo.tm_hour>7 and not (i==0 and tiempo.tm_mday==day)) or i>0:
		...
		while j<=n and mess_count<=4:
			...
			null_pading_flight_search_container="flight_info_legend"  		#En HTML aparece contenedor con esta clase para mostrar que no hay pasajes en fecha indicada

			len_elem=0
			count=0	

			message_container=browser.find_elements_by_class_name(null_pading_flight_search_container)
			x=browser.find_elements_by_xpath("//div[@data-bind='slideIn: noFlightsFound']") #try exception investigar
			try:
