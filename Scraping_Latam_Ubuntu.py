...
while True:
	if (tiempo.tm_hour<11 and tiempo.tm_hour>7 and not (i==0 and tiempo.tm_mday==day)) or i>0:
		day=tiempo.tm_mday
		Active=True
		...
		with open('Vuelos '+str_hoy.replace('/','-')+'.csv', mode='a') as vuelos_file:
		    ...
		    vuelos_writer.writerow(['Código','Hora salida','Origen','Ciudad Origen','Hora llegada','Info1','Info2','Destino','Ciudad Destino','Modo','Duración','Tarifa','Fecha Vuelo','Update','Disponibilidad'])
		'''
			...
			while (len_elem==0 and count<1000) and len(message_container)==0:
				...
				for el in elem:
					...
					with open('Vuelos '+str_hoy.replace('/','-')+'.csv', mode='a') as vuelos_file:
						...
						hora_salida=el_arr[0]+el_arr[1]
						iata_origen=el_arr[2]
						hora_llegada=el_arr[3]+el_arr[4]
						if len(el_arr)==10:
							iata_destino=el_arr[5]
							modo=el_arr[6]
							duration=el_arr[7]
							tarifa_vuelo=el_arr[9]
							...
							iata_destino=el_arr[7].encode('utf-8')
							modo=el_arr[8].encode('utf-8')
							duration=el_arr[9].encode('utf-8')
							tarifa_vuelo=el_arr[11].encode('utf-8')
							vuelos_writer.writerow([el.get_attribute("id"),hora_salida,iata_origen,vuelos_origen,hora_llegada,el_arr[5].encode('utf-8'),el_arr[6].encode('utf-8'),iata_destino,vuelos_destino,modo,duration,tarifa_vuelo,vuelos_fecha_salida_ddmmaaaa,datetime.now(),""])
							pass

						elif len(el_arr)==11:
							iata_destino=el_arr[5].encode('utf-8')
							modo=el_arr[6].encode('utf-8')
							duration=el_arr[7].encode('utf-8')
							tarifa_vuelo=el_arr[9].encode('utf-8')
							vuelos_writer.writerow([el.get_attribute("id"),hora_salida,iata_origen,vuelos_origen,hora_llegada,"el mismo dia","",iata_destino,vuelos_destino,modo,duration,tarifa_vuelo,vuelos_fecha_salida_ddmmaaaa,datetime.now(),el_arr[10].encode('utf-8')])
							pass
						elif len(el_arr)==13:
							iata_destino=el_arr[7].encode('utf-8')
							modo=el_arr[8].encode('utf-8')
							duration=el_arr[9].encode('utf-8')
							tarifa_vuelo=el_arr[11].encode('utf-8')
							vuelos_writer.writerow([el.get_attribute("id"),hora_salida,iata_origen,vuelos_origen,hora_llegada,el_arr[5].encode('utf-8'),el_arr[6].encode('utf-8'),iata_destino,vuelos_destino,modo,duration,tarifa_vuelo,vuelos_fecha_salida_ddmmaaaa,datetime.now(),el_arr[12].encode('utf-8')])
							pass
						else:
							print("No se almaceno ningún dato")
